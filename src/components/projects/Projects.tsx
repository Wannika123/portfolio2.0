...
export default function Projects() {
    const sortedProjects = projects.toReversed();  // from new to old

    const { selectedVals } = useContext(SelectContext);

    const [endingIndex, setEndingIndex] = useState(6);

    let displayedProjects: ProjectType[] = [];

    if (selectedVals.length === 0) {
        displayedProjects = [
            ...sortedProjects.filter(project => project.feature), 
            ...sortedProjects.filter(project => !project.feature)
        ];   // feature projects appear first
    } else {
        displayedProjects = sortedProjects.filter(project => {
            let condition = true
